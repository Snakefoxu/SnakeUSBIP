...
Write-Host "Starting SnakeUSBIP v$version Release Build..." -ForegroundColor Cyan
...
# Helper tools (from Portable folder restored from GitHub)
...
# Drivers (from Portable/drivers folder)
$driversPath = "$repoRoot\Portable\drivers"
if (Test-Path $driversPath) {
    Copy-Item $driversPath "$releasePath\temp_x64\Portable\drivers" -Recurse -Force
    Copy-Item $driversPath "$releasePath\temp_arm64\Portable\drivers" -Recurse -Force
}
...
# Icon for installer
$iconSrc = "$projectPath\Assets\icon.ico"
if (Test-Path $iconSrc) { Copy-Item $iconSrc "$releasePath\temp_x64\icon.ico" -Force }
...
Write-Host "Creating ZIP packages..." -ForegroundColor Magenta
$x64Zip = "$releasePath\SnakeUSBIP-v$version-x64.zip"
$arm64Zip = "$releasePath\SnakeUSBIP-v$version-arm64.zip"

# Create .portable marker for ZIP releases (Hybrid Mode)
New-Item -Path "$releasePath\temp_x64\Portable\.portable" -ItemType File -Force | Out-Null
New-Item -Path "$releasePath\temp_arm64\Portable\.portable" -ItemType File -Force | Out-Null
...
if (Test-Path $iscc) {
    Write-Host "Compiling Setup (x64)..." -ForegroundColor Yellow
    
    # Remove .portable marker for installer (uses AppData instead)
    if (Test-Path "$releasePath\temp_x64\Portable\.portable") {
        Remove-Item "$releasePath\temp_x64\Portable\.portable" -Force
    }
    ...
    if ($proc.ExitCode -eq 0) { Write-Host "Setup (x64) created successfully" -ForegroundColor Green }
